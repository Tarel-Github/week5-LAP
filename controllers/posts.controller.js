...
class PostsController {
    ...
    likePost = async(req,res,next)=>{
        const { postId } = req.params;
        const user = res.locals.user;
        const likePost = await this.postService.likePost(res, postId,user.userId)
    ...
    likeGet = async(req,res,next) => {
        const user = res.locals.user;
        const likeGet = await this.postService.likeGet(res, user.userId)
        res.status(200).json({data: likeGet})
    }
}
